import random, os, httpx, subprocess, asyncio, json, re, time
...
async def _fetch_one_source(name, meta):
    ...
    try:
        ...
        if resp.status_code in (301, 302, 303, 307, 308):
            ...
            print(f"[WARN] source='{name}' 302 but no Location header", flush=True)
            return None
        ...
        if mode == "json" or (mode == "auto" and "json" in ct):
            ...
            if data is not None:
                ...
                if vu:
                    ...
                    return {"token": token, "name": disp, "remote": True}
                print(f"[WARN] source='{name}' json_path='{json_path}' extraction FAILED", flush=True)
                return None
        ...
        if mode == "text_url" or (mode == "auto" and ct.startswith("text/plain")):
            ...
            if line and ("http://" in line or "https://" in line):
                ...
                return {"token": token, "name": disp, "remote": True}
            print(f"[WARN] source='{name}' text_url body has no valid URL", flush=True)
            return None
        ...
        if mode == "html" or mode == "auto":
            ...
            for pat in [r'src="([^"]*\.mp4[^"]*)"', r"src='([^']*\.mp4[^']*)'",
                ...
                if srcs:
                    ...
                    token = register_remote(vu, disp)
                    return {"token": token, "name": disp, "remote": True}
            print(f"[WARN] source='{name}' html mode: no <video src> matched", flush=True)
            return None

        print(f"[WARN] source='{name}' mode={mode} ct={ct[:40]} - no matching branch (fallthrough)", flush=True)
        return None
    except httpx.TimeoutException:
        print(f"[WARN] source='{name}' TIMEOUT (>6000ms)", flush=True)
        return None
    except httpx.HTTPError as he:
        print(f"[WARN] source='{name}' HTTPError {type(he).__name__}: {he}", flush=True)
        return None
    except Exception as e:
        print(f"[WARN] source='{name}' EXCEPTION {type(e).__name__}: {e}", flush=True)
        return None


async def _fetch_group(group, max_attempts=None):
    """聚合分发器: 从 group 内按权重随机选未熔断源,失败自动降级到下一个,直到成功或全部失败.
    仅在整组耗尽时打 1 行 ERROR 日志,中途不打.
    """
    tried = set()
    total = 0
    t_start = time.time()
    ...
    print(f"[ERROR] group='{group}' EXHAUSTED after {total} attempts ({dt}ms), tried={sorted(tried)}", flush=True)
    return None
...
async def api_random(source: str | None = None):
    ...
    if group_names:
        ...
        result = await _fetch_group(chosen)
        if result:
            return result
    # 3. 还不行 -> 试独立远程源
